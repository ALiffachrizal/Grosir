...
<div class="max-w-lg mx-auto">
    <div class="bg-white rounded-xl shadow p-6">
        <form action="{{ route('suppliers.update', $supplier) }}" method="POST" class="space-y-5">
            ...
            {{-- Hidden kode supplier --}}
            <input type="hidden" name="kode_supplier" value="{{ old('kode_supplier', $supplier->kode_supplier) }}">
            ...
            <div>
                ...
                <select name="category_id"
                    ...
                    @foreach($supplierCategories as $cat)
                        <option value="{{ $cat->id }}"
                            {{ old('category_id', $supplier->category_id) == $cat->id ? 'selected' : '' }}>
                            {{ $cat->name }}
                        </option>
                    @endforeach
                </select>
